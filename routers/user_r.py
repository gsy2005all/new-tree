...
def set_user_token_to_user_public(user: User) -> UserPublic:
...
# 定义一个函数，参数x是一个usercreate对象，db_handelr是一个可以操作数据库的对象
def add_user(user_n: User, db_handelr: DbHandelr):  
    # 将User对象的password属性设置为哈希密码
    user_n.password = get_passwd_hash(user_n.password) 
    # 将User对象添加到数据库会话中
    db_handelr.add(user_n) 
    # 提交数据库会话，将添加的User对象保存到数据库中
    db_handelr.commit() 
    # 刷新数据库会话中的User对象，获取数据库中生成的id等字段的值
    db_handelr.refresh(user_n) 
...
def login_user(user_n: User, db_handelr: DbHandelr):
    # 查询数据库中是否存在用户名为user_n.name的User对象
    found_user = db_handelr.exec(select(User).where(User.name == user_n.name)).first()
...
def token_login(request: Request, db_handelr: DbHandelr):
    # 从请求状态中获取用户负载信息
    ...
    found_user = db_handelr.get(User, user_payload.user_id) 
